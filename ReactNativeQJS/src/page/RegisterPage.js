...
class RegisterPage extends Component {
    ...
    componentWillUnmount() {
        ...
        this.props.navigation.dispatch(RegisterAction.setMessageCodeTxt('获取验证码'));
    }
...
//验证手机号只能输入数字
    ...
    register = () => {
        ...
        register(this.props.validPhone, this.props.validPwd, messageCode);
    }

    isShowRegisterPassWord = () => {
        let {isShowRegisterPassWord} = this.props;
        isShowRegisterPassWord(!this.props.isShowPwd)
    }
    ...
    timeCountDown = () => {
        ...
        this.interval = setInterval(() => {
            if (time < 1) {
                this.props.navigation.dispatch(RegisterAction.setMessageCodeTxt('重新获取'));
                this.interval && clearInterval(this.interval)
            } else {
                this.props.navigation.dispatch(RegisterAction.setMessageCodeTxt(time + 's'));
                time--;
            }
        }, 1000);
    }
    ...
    render() {
        return <View style={styles.container}>
            ...
            <CustomTextInput
                ...
                iconLeftStyle={styles.iconLeftStyle}
                borderBottomColor={Colors.bottomColor}
                iconRightOnPress={this.isShowRegisterPassWord}
                placeholderTextColor={Colors.textColorHint}
                maxLength={18}
...
const mapStateToProps = (state) => {
    let {validPhone, validPwd, isShowPwd, messageCodeTxt, errInfo, isLoading,success} = state.register;
    return {
        messageCodeTxt,
        errInfo: errInfo,
        isLoading: isLoading,
...
const mapDispatchToProps = (dispatch) => {
    return {
        register: (phone, password, messageCode) => {
            if (!TextUtils.isPhoneNumber(phone)) {
                ...
                dispatch(RegisterAction.register(phone, password, messageCode));
            }
        },
        getValidPhone: (inputPhone) => {
            let validPhone = inputPhone.replace(/\D/g, '');
            dispatch(RegisterAction.getValidPhone(validPhone))
        },
        getValidPwd: (inputPwd) => {
            let validPwd = inputPwd.replace(/[^\a-\z\A-\Z0-9\_]/g, '');//只能输入数字和字母以及下划线
            dispatch(RegisterAction.getValidPwd(validPwd))
        },
        isShowRegisterPassWord: (isShowPwd) => {
            dispatch(RegisterAction.isShowRegisterPassWord(isShowPwd))
        }
    };
}
